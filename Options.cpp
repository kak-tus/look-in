#include <windows.h>
#include "../Miranda/miranda/include/newpluginapi.h"
#include "../Miranda/miranda/include/m_langpack.h"
#include "../Miranda/miranda/include/m_database.h"
#include "resource.h"
#include "../Miranda/miranda/include/m_options.h"
#include "../Miranda/miranda/include/m_protocols.h"
#include <string.h>
#include "Const.h"
  ...
  switch (Msg) {
    case WM_INITDIALOG: {
      ...
      for (int i = 0; i < pptSize; i++) {
        if (prt[i]->type == PROTOTYPE_PROTOCOL) {
          ...
          if (!DBGetContactSetting(0, "look-in", s, &dbv)) {
            ...
            ppt[i].templ = strdup(intTempl[intTemplSz - 1].name);
          }
      ...
      for (i = 0; i < intTemplSz; i++) { //добавление встроенных в dll шаблонов
        SendDlgItemMessage(hWnd, IDC_TEMPLATE, CB_ADDSTRING, 0, (LPARAM)intTempl[i].name);
      }
      ...
      switch LOWORD(wParam) {
        ...
        case IDC_PLUGIN: {
          switch HIWORD(wParam) {
            case LBN_SELCHANGE: {
              ...
              if (m == CB_ERR) {
                m = SendDlgItemMessage(hWnd, IDC_TEMPLATE, CB_FINDSTRING, -1, (LPARAM)intTempl[intTemplSz - 1].name);
                if (m == CB_ERR) {
                  m = -1;
                }
              }
        ...
        case IDC_PROTOCOL: {
          switch HIWORD(wParam) {
            case CBN_SELCHANGE: {
              int n = SendDlgItemMessage(hWnd, IDC_PLUGIN, LB_GETCURSEL, 0, 0);
              int m = SendDlgItemMessage(hWnd, IDC_PROTOCOL, CB_GETCURSEL, 0, 0);
              if (n == LB_ERR || m == LB_ERR) {
                break;
              }
              ppt[n].protocol = m;
              PostMessage(GetParent(hWnd), PSM_CHANGED, 0, 0);
        ...
        case IDC_TEMPLATE: {
          switch HIWORD(wParam) {
            case CBN_SELCHANGE: { //изменено поле шаблонов
              int n = SendDlgItemMessage(hWnd, IDC_PLUGIN, LB_GETCURSEL, 0, 0);
    ...
    case WM_NOTIFY: {
      ...
      case 0:
        ...
        case PSN_APPLY: //нажата кнопка "Apply" или "OK"

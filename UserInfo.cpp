...
#include <stdio.h>
#include "../Miranda/miranda/include/statusmodes.h"
#include "Const.h"
#include "Template.h"
#include "resource.h"
#include "../Miranda/miranda/include/m_userinfo.h"
#include "../Miranda/miranda/include/m_system.h"
#include <time.h>
#include "../Miranda/miranda/include/m_utils.h"
...
static void showUserInfo(HWND hWnd, HANDLE hContact) {
  ...
  if (!DBGetContactSetting(0, "look-in", s, &dbv)) {
    ...
    templ = strdup(intTempl[intTemplSz - 1].name);
  }
  free(s);
  ...
  for (int i = 0; i < intTemplSz; i++) {
    if (strcmp(templ, intTempl[i].name) == 0) {
      b = true;
      tmpl = strdup(Translate(intTempl[i].templ));
      break;
    }
  }
  ...
  for (i = 0; i < protocol[proto].listSize; i++) {
    ...
    switch (ili[i].DBType) {
      ...
      case ILCOUNTRY: {
        char *v = NULL;
        if (!DBGetContactSetting(hContact, prt, ili[i].DBName, &dbv)) {
            char *s = (char*)CallService(MS_UTILS_GETCOUNTRYBYNUMBER, dbv.wVal, 0);
            if (s) {
              v = strdup(Translate(s));
            }
          DBFreeVariant(&dbv);
        }
        setVariable(&tmpl, ili[i].DBName, v);
